import json

host_nations = ['United States', 'Mexico', 'Canada']

partite_reali = {
    ('Mexico', 'South Africa'): (2, 0),
    ('South Korea', 'Czechia'): (2, 1),
    ('Canada', 'Bosnia and Herzegovina'): (1, 1),
    ('United States', 'Paraguay'): (4, 1),
    ('Qatar', 'Switzerland'): (1, 1),
    ('Brazil', 'Morocco'): (1, 1),
    ('Haiti', 'Scotland'): (0, 1),
    ('Australia', 'Turkey'): (2, 0),
    ('Germany', 'Curaçao'): (7, 1),
    ('Netherlands', 'Japan'): (2, 2),
    ('Ivory Coast', 'Ecuador'): (1, 0),
    ('Sweden', 'Tunisia'): (5, 1),
    ('Spain', 'Cape Verde'): (0, 0),
    ('Belgium', 'Egypt'): (1, 1),
    ('Saudi Arabia', 'Uruguay'): (1, 1),
    ('Iran', 'New Zealand'): (2, 2),
    ('France', 'Senegal'): (3, 1),
    ('Iraq', 'Norway'): (1, 4),
    ('Argentina', 'Algeria'): (3, 0),
    ('Austria', 'Jordan'): (3, 1),
    ('Portugal', 'DR Congo'): (1, 1),
    ('England', 'Croatia'): (4, 2),
    ('Ghana', 'Panama'): (1, 0),
    ('Uzbekistan', 'Colombia'): (1, 3),
    ('Czechia', 'South Africa'): (1, 1),
    ('Switzerland', 'Bosnia and Herzegovina'): (4, 1),
    ('Canada', 'Qatar'): (6, 0),
    ('Mexico', 'South Korea'): (1, 0),
    ('United States', 'Australia'): (2, 0),
    ('Scotland', 'Morocco'): (0, 1),
    ('Brazil', 'Haiti'): (3, 0),
    ('Turkey', 'Paraguay'): (0, 1),
    ('Netherlands', 'Sweden'): (5, 1),
    ('Germany', 'Ivory Coast'): (2, 1),
    ('Ecuador', 'Curaçao'): (0, 0),
    ('Tunisia', 'Japan'): (0, 4),
    ('Spain', 'Saudi Arabia'): (4, 0),
    ('Belgium', 'Iran'): (0, 0),
    ('Uruguay', 'Cape Verde'): (2, 2),
    ('New Zealand', 'Egypt'): (1, 3),
    ('Argentina', 'Austria'): (2, 0),
    ('France', 'Iraq'): (3, 0),
    ('Norway', 'Senegal'): (3, 2),
    ('Jordan', 'Algeria'): (1, 2),
    ('Portugal', 'Uzbekistan'): (5, 0),
    ('England', 'Ghana'): (0, 0),
    ('Panama', 'Croatia'): (0, 1),
    ('Colombia', 'DR Congo'): (1, 0),
    ('Bosnia and Herzegovina', 'Qatar'): (3, 1),
    ('Switzerland', 'Canada'): (2, 1),
    ('Morocco', 'Haiti'): (4, 2),
    ('Scotland', 'Brazil'): (0, 3),
    ('South Africa', 'South Korea'): (1, 0),
    ('Czechia', 'Mexico'): (0, 3),
    ('Curaçao', 'Ivory Coast'): (0, 2),
    ('Ecuador', 'Germany'): (2, 1),
    ('Tunisia', 'Netherlands'): (1, 3),
    ('Japan', 'Sweden'): (1, 1),
    ('Paraguay', 'Australia'): (0, 0),
    ('Turkey', 'United States'): (3, 2),
    ('Norway', 'France'): (1, 4),
    ('Senegal', 'Iraq'): (5, 0),
    ('Cape Verde', 'Saudi Arabia'): (0, 0),
    ('Uruguay', 'Spain'): (0, 1),
    ('New Zealand', 'Belgium'): (1, 5),
    ('Egypt', 'Iran'): (1, 1),
    ('Panama', 'England'): (0, 2),
    ('Croatia', 'Ghana'): (2, 1),
    ('Colombia', 'Portugal'): (0, 0),
    ('DR Congo', 'Uzbekistan'): (3, 1),
    ('Algeria', 'Austria'): (3, 3),
    ('Jordan', 'Argentina'): (1, 3),
    ('South Africa', 'Canada'): (0, 1),
    ('Brazil', 'Japan'): (2, 1),
    ('Germany', 'Paraguay'): (1, 1),
    ('Netherlands', 'Morocco'): (1, 1),
    ('Ivory Coast', 'Norway'): (1, 2),
    ('France', 'Sweden'): (3, 0),
    ('Mexico', 'Ecuador'): (2, 0),
    ('England', 'DR Congo'): (2, 1),
    ('Belgium', 'Senegal'): (3, 2),
    ('United States', 'Bosnia and Herzegovina'): (2, 0),
    ('Spain', 'Austria'): (3, 0),
    ('Portugal', 'Croatia'): (2, 1),
    ('Switzerland', 'Algeria'): (2, 0),
    ('Australia', 'Egypt'): (1, 1),
    ('Argentina', 'Cape Verde'): (3, 2),
    ('Colombia', 'Ghana'): (1, 0),
    ('Canada', 'Morocco'): (0, 3),
    ('Paraguay', 'France'): (0, 1),
    ('Brazil', 'Norway'): (1, 2),
    ('Mexico', 'England'): (2, 3),
    ('Portugal', 'Spain'): (0, 1),
    ('United States', 'Belgium'): (1, 4),
    ('Argentina', 'Egypt'): (3, 2),
    ('Switzerland', 'Colombia'): (0, 0),
}

latest_elo = {
    'Argentina': 2156, 'Spain': 2177, 'France': 2143, 'England': 2076, 
    'Brazil': 1993, 'Colombia': 2003, 'Portugal': 1995, 'Netherlands': 1971, 
    'Norway': 1972, 'Germany': 1908, 'Switzerland': 1949, 'Mexico': 1913, 
    'Japan': 1888, 'Ecuador': 1871, 'Croatia': 1882, 'Morocco': 1921, 
    'Denmark': 1869, 'Italy': 1869, 'Belgium': 1961, 'Turkey': 1852, 
    'Uruguay': 1841, 'Austria': 1821, 'Senegal': 1816, 'Paraguay': 1814, 
    'Australia': 1795, 'United States': 1747, 'Algeria': 1785, 'Ukraine': 1780, 
    'Russia': 1772, 'Nigeria': 1767, 'Iran': 1764, 'Canada': 1729, 
    'Scotland': 1745, 'Greece': 1744, 'Ivory Coast': 1727, 'Sweden': 1731, 
    'Egypt': 1742, 'Serbia': 1734, 'Venezuela': 1733, 'South Korea': 1723, 
    'Chile': 1717, 'Kosovo': 1714, 'Hungary': 1710, 'Poland': 1710, 
    'Peru': 1699, 'Ireland': 1699, 'Wales': 1682, 'Slovenia': 1682, 
    'Czechia': 1680, 'Uzbekistan': 1631, 'Panama': 1658, 'Slovakia': 1667, 
    'DR Congo': 1704, 'Georgia': 1654, 'Israel': 1647, 'Romania': 1639, 
    'Jordan': 1628, 'Cape Verde': 1619, 'Bosnia and Herzegovina': 1605, 
    'Bolivia': 1621, 'Albania': 1616, 'Cameroon': 1614, 'Costa Rica': 1608, 
    'Northern Ireland': 1605, 'Saudi Arabia': 1596, 'North Macedonia': 1589, 
    'Mali': 1588, 'Iraq': 1561, 'Ghana': 1570, 'South Africa': 1559, 
    'Honduras': 1570, 'Iceland': 1568, 'Tunisia': 1562, 'New Zealand': 1534, 
    'Angola': 1542, 'United Arab Emirates': 1540, 'Finland': 1536, 
    'Burkina Faso': 1529, 'Jamaica': 1527, 'Belarus': 1522, 'Haiti': 1517, 
    'Guatemala': 1505, 'Oman': 1479, 'Syria': 1479, 'Palestine': 1465, 
    'Guinea': 1463, 'Montenegro': 1461, 'Bulgaria': 1458, 'Luxembourg': 1450, 
    'Northern Cyprus': 1442, 'Curaçao': 1438, 'Suriname': 1431, 
    'Kazakhstan': 1428, 'China': 1424, 'Kurdistan': 1424, 'Libya': 1420, 
    'Gambia': 1419, 'Bahrain': 1414, 'Qatar': 1411
}

gironi_ufficiali = {
    'A': ['Mexico', 'South Africa', 'South Korea', 'Czechia'],
    'B': ['Switzerland', 'Canada', 'Bosnia and Herzegovina', 'Qatar'],
    'C': ['Brazil', 'Morocco', 'Scotland', 'Haiti'],
    'D': ['United States', 'Australia', 'Paraguay', 'Turkey'],
    'E': ['Germany', 'Ivory Coast', 'Ecuador', 'Curaçao'],
    'F': ['Netherlands', 'Japan', 'Sweden', 'Tunisia'],
    'G': ['Egypt', 'Iran', 'Belgium', 'New Zealand'],
    'H': ['Spain', 'Uruguay', 'Cape Verde', 'Saudi Arabia'],
    'I': ['France', 'Norway', 'Senegal', 'Iraq'],
    'J': ['Argentina', 'Austria', 'Algeria', 'Jordan'],
    'K': ['Colombia', 'Portugal', 'DR Congo', 'Uzbekistan'],
    'L': ['England', 'Ghana', 'Croatia', 'Panama']
}

# Trasformazione delle chiavi di partite_reali da tuple a stringhe per il JSON
partite_reali_json = {f"{k[0]}|{k[1]}": v for k, v in partite_reali.items()}

# Definizione di un dizionario che conterrà tutti i dati da esportare
config_data = {
    "host_nations": host_nations,
    "partite_reali": partite_reali_json,
    "latest_elo": latest_elo,
    "gironi_ufficiali": gironi_ufficiali
}

# Esporto dei dati in un file JSON
with open('config_mondiale.json', 'w') as f:
    json.dump(config_data, f, indent=4)

print("✅ File config_mondiale.json creato con successo!")